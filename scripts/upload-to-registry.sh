...
try_login() {
  ...
  if [ ! -f ${marker} ] && [ -v ${token} ]; then
    printenv ${token} | docker login ${registry} --username ${user} --password-stdin || return 1
    touch ${marker}
  fi
}
...
upload_images() {
  ...
  try_login ${registry} ${user} ${token} ${marker} || echo "Failed 'docker login ${registry} --username ${user}'"
